...
class ScrollContainer extends React.Component {
  constructor() {
    ...
    this.atHead = this.atHead.bind(this)
  ...
  handleOnWheel(e) {
    if (e.deltaY > 0 && this.state.allow === false && this.state.active !== this.props.children.length - 1) { this.moveDown() }
    if (e.deltaY < 0 && this.state.allow === false && this.state.active !== 0) { this.moveUp() }
  }
      ...
      <div>
        <ul className="onepagePagination">
          {_.map(_.times(this.props.children.length, Number), (i) => {
            return (
              <li>
                <a className={this.state.active === i ? "active" : ""}
